...
from app.services.notification_templates import (
    ASSIGNMENT_ADDED_REVIEW_TEMPLATE_KEY,
    ASSIGNMENT_ADDED_SAMPLE_TEMPLATE_KEY,
...
def _notify_assignment_added(
    ...
    *,
    ...
    order_event_id: UUID,
    template_key: str,
    extra_template_params: Optional[Dict[str, str]] = None,
) -> int:
    ...
    `template_key` is passed in by the caller rather than derived from
    `CURRENT_TEMPLATE_KEY[ASSIGNMENT_ADDED]` here: that map is 1:1 per
    `NotificationType` and this one type now has three valid contexts
    (order/sample/review), each with its own copy — see
    `notification_templates.py`'s `ASSIGNMENT_ADDED_*_TEMPLATE_KEY` constants.
...
def notify_order_reviewers_added(
    ...
    *,
    ...
    Pre-release remediation: uses `assignment_added_review_v1`, not the
    order-assignee key, so the copy says "revisión asignada" rather than the
    generic "te asignó a esta orden" — the two are different facts and were
    previously indistinguishable in the Notification Center.
    """
    return _notify_assignment_added(
        session,
        tenant_id=order.tenant_id,
        ...
        actor=actor,
        order_event_id=order_event_id,
        template_key=ASSIGNMENT_ADDED_REVIEW_TEMPLATE_KEY,
    )
...
def notify_sample_assignments_added(
    ...
    *,
    ...
    Pre-release remediation: uses `assignment_added_sample_v1`, which adds a
    `sample_code` parameter, so the copy names the sample explicitly ("te
    asignó la muestra M-02 de esta orden") instead of the generic
    order-assignee copy the sample context previously reused.
    """
    return _notify_assignment_added(
        session,
        ...
        actor=actor,
        order_event_id=order_event_id,
        template_key=ASSIGNMENT_ADDED_SAMPLE_TEMPLATE_KEY,
        extra_template_params={"sample_code": sample.sample_code},
    )
